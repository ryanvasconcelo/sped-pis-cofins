...
import { parseExtratoBancario } from './src/lib/extrato-financeiro/extrato-parser.js';
...
async function test(file, name) {
    ...
    try {
        const r3 = await parseExtratoBancario(b, name);
        console.log('parseExtratoBancario ->', r3.layoutDetectado);
    } catch(e) { console.log('parseExtratoBancario error:', e.message); }
}
